                            ...
                            <div className="flex items-center gap-4">
                                    ...
                                    {p.previewUrl ? (
                                        <>
                                            <img
                                                ...
                                                style={{ transform: `rotate(${Number(p.rotation ?? "0")}deg)` }}
                                            />
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        {/* 写真选择 / 変更 */}
                                        <label
                                            className={`inline-flex items-center gap-2 px-3 py-2 rounded-xl border cursor-pointer transition-colors text-sm
      ${p.uploading
                                                    ? "bg-gray-100 border-gray-200 text-gray-400 cursor-not-allowed"
                                                    : "bg-gray-50 border-gray-200 hover:border-[#c0a062] text-gray-700"}
    `}
                                        >
                                            <Upload size={16} />
                                        ...
                                        {p.previewUrl && (
                                            <button
                                                ...
                                                disabled={p.uploading}
                                                onClick={() => {
                                                    const next = Number(p.rotation ?? "0") + 90; //TO-DO 保存 / 提交给后端时再规整一次：const normalized = rotation % 360;
                                                    updatePhotoField(p.id, "rotation", String(next));
                                                }}
                    ...
                    {p.file && (
                        <div className="p-4 md:p-5 bg-gray-50 border-t border-gray-200 animate-fade-in-up">
                            <div className="grid md:grid-cols-2 gap-4">
                                ...
                                <div className="md:col-span-2">
                                    <label className="block text-sm font-bold text-gray-700 mb-2">写真のエピソード（任意）</label>
                                    <textarea rows={2} value={p.photo_description} onChange={(e) => updatePhotoField(p.id, "photo_description", e.target.value)} placeholder="撮影時の思い出や、この写真に込めた気持ちなど..." className="w-full px-4 py-2.5 rounded-xl bg-white border border-gray-200 focus:border-[#c0a062] focus:ring-2 focus:ring-[#c0a062]/20 outline-none resize-none text-sm" />
                                </div>
                            </div>
                        </div>
                    )}
